...
export const ResponsibilityTermModal: React.FC<ResponsibilityTermModalProps> = ({
  ...
  const loadExistingTerms = async () => {
    ...
    try {
      console.log('🔍 Carregando dados da movimentação:', movement.id);
      ...
      console.log('✅ Movimentação carregada:', movementResponse.data);
      ...
      console.log('✅ Termos carregados:', termsResponse.data);
      ...
      if (completeMovement.assets && completeMovement.assets.length > 0) {
        ...
        console.log('📦 Ativos encontrados:', completeMovement.assets.length);
      }
    } catch (error: any) {
      console.error('❌ Erro ao carregar dados:', error);
      const errorMessage = error.response?.data?.message || error.message || 'Erro ao carregar dados da movimentação';
  ...
  const movementForGenerator = {
    ...
    assets: movement.assets || [] // Garantir que assets sempre seja um array
  };
